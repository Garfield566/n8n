...
⚠️ EXCEPTION : NOTIONS MATHÉMATIQUES
Pour les notions mathématiques (fonctions, théorèmes, concepts math), tu dois :

Analyser les graphiques TikZ existants dans les exemples fournis par l'utilisateur
Identifier le type de graphique approprié :

Fonction sur grille cartésienne (comme logarithme, exponentielle)
Triangle/figure géométrique (comme pour sinus/cosinus)
Cercle trigonométrique
Diagramme/schéma explicatif


Réutiliser et adapter un graphique TikZ qui FONCTIONNE déjà
Ne JAMAIS inventer un nouveau code TikZ de zéro
Modifier uniquement :

Le domaine (domain=)
La fonction tracée (plot (\x,{...}))
Les labels et légendes
L'échelle si nécessaire (scale=)



Exemples de graphiques de référence :
